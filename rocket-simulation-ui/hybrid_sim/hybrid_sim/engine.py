...
class EngineModel:
    ...
    def _liquid_mass(self, m_ox, T):
        return eq.tank_liquid_mass(float(m_ox), float(T), self.e.V_tank)

    def _cstar(self, OF):
        return eq.characteristic_velocity(
            float(OF), CSTAR_OF, CSTAR_VAL,
            self._fuel.of_shift, self._fuel.cstar_scale)

    def _mdot_ox(self, m_l, T, Pc):
        # No liquid left means no injector flow, whatever the pressures say.
        if m_l <= self.c.eps_mass:
            return 0.0, eq.n2o_saturation_pressure(float(T))
        return eq.injector_mass_flow(
            self.e.Cd_inj*self.e.A_inj, float(T), float(Pc),
            self.c.kappa, self.c.Pc_to_Ptank_max)
    ...
    def _mdot_vent(self, T, Pt):
        ...
        """
        return eq.vent_mass_flow(self.e.Cd_vent, self.e.A_vent,
                                 float(Pt), float(T))

    def _cf(self, Pc, eps=None, Me=None):
        if eps is None: eps = self.e.eps_exp
        if Me is None: Me = self._Me
        return eq.thrust_coefficient(float(Pc), self.Pa, eps, Me,
                                     self.e.gamma, self.c.sep_criterion)

    def _pc_target(self, mdot_tot, OF, Pt, At):
        return eq.chamber_pressure_target(
            mdot_tot, self._cstar(OF), At, self.e.eta_cstar,
            Pt, self.c.Pc_to_Ptank_max)
    ...
    def _rhs(self, t, y):
        ...
        if r_port < e.R_outer and m_f > c.eps_mass and mdot_ox > 0:
            G = eq.oxidiser_flux(mdot_ox, e.A_port(r_port))
            rdot = eq.regression_rate(G, self._fuel.a, self._fuel.n)
            mdot_f = eq.fuel_mass_flow(self._fuel.rho, e.A_burn(r_port), rdot)
        else:
            rdot = mdot_f = 0.0
        ...
        if m_l > c.eps_mass:
            ...
            dT = eq.n2o_tank_cooling_rate(m_l, float(T),
                                          mdot_ox + mdot_vent, self._cool)
        else:
            dT = 0.0

        dPc = eq.chamber_pressure_lag(
            self._pc_target(mdot_tot, OF, Pt, At), Pc, c.tau_chamber)
